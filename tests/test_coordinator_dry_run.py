...
async def test_timer_countdown_uses_remaining_attribute(hass) -> None:
    from pytest_homeassistant_custom_component.common import MockConfigEntry
    ...
    hass.states.async_set("climate.test", "off")
    hass.states.async_set("timer.test", "active", {"remaining": "0:04:59"})
    ...
    data = await coordinator._evaluate("poll")
    assert data.timer_countdown == "0:04:59"


async def test_timer_countdown_is_off_when_timer_idle(hass) -> None:
    from pytest_homeassistant_custom_component.common import MockConfigEntry
    ...
    data = await coordinator._evaluate("poll")
    assert data.timer_countdown == "Off"
