...
def test_adjust_disabled_state():
    ...
    assert adjust(TEST_PARAMS, home, state).output is HomeOutput.DISABLED


def test_adjust_disabled_resets_tolerated():
    state = CachedState(tolerated=5)
    assert adjust(TEST_PARAMS, default_input(enabled=False), state).output is HomeOutput.DISABLED
...
def test_adjust_disabled_last_disabled_returns_no_change():
    ...
    assert adjust(TEST_PARAMS, default_input(enabled=False), state).output is HomeOutput.NO_CHANGE
...
def test_manual_cool_grid_usage_sets_timer():
    ...
    assert adjust(TEST_PARAMS, home, state).output is HomeOutput.TIMER
...
def test_scenario_timer_lifecycle_manual_to_timer_to_reset():
    ...
    assert adjust(TEST_PARAMS, home, state).output is HomeOutput.RESET
...
# --- reason label tests (formerly explain() tests) ---


def test_reason_manual_no_timer():
    """Manual mode on grid → reason is just 'Manual'."""
    home = default_input(aircon_mode=AirconMode.COOL, grid_usage=0.1, timer=False)
    assert adjust(TEST_PARAMS, home, CachedState()).reason == "Manual"


def test_reason_manual_timer_active():
    """Manual mode with timer already running → 'No change'."""
    home = default_input(aircon_mode=AirconMode.COOL, grid_usage=0.1, timer=True)
    assert adjust(TEST_PARAMS, home, CachedState()).reason == "No change"
...
def test_reason_grid_usage_too_high():
    ...
    assert adjust(TEST_PARAMS, home, state).reason == "Grid usage too high"


def test_reason_grid_usage_tolerated():
    """Auto mode, grid usage within tolerance → 'Grid usage tolerated'."""
    home = default_input(aircon_mode=AirconMode.COOL, auto=True, grid_usage=100.0, generation=0.0)
    assert adjust(TEST_PARAMS, home, CachedState(tolerated=0)).reason == "Grid usage tolerated"


def test_reason_disabled():
    assert adjust(TEST_PARAMS, default_input(enabled=False), CachedState()).reason == "Disabled"


def test_reason_auto_idle():
    home = default_input(auto=True, generation=0.0)
    assert adjust(TEST_PARAMS, home, CachedState()).reason == "Auto idle"
