"""Tests for number entity registration, state, and coordinator parameter overrides."""
...
async def test_number_entities_register_on_setup(hass, loaded_entry) -> None:
    """All number entities should be created when the config entry is set up."""
    entity_ids = [
...
async def test_number_entities_show_default_values(hass, loaded_entry) -> None:
    """Number entities should show code defaults when no options are set."""
...
async def test_set_number_entity_updates_parameter(hass, loaded_entry) -> None:
    """Setting a number entity value should update the coordinator parameter."""
...
async def test_get_parameter_falls_back_to_options(coord_factory) -> None:
    """get_parameter should use options when no override is set."""
    from custom_components.home_rules.const import (
...
async def test_get_parameter_override_takes_precedence(coord_factory) -> None:
    """Runtime override should take precedence over config entry options."""
    from custom_components.home_rules.const import (
        CONF_TEMPERATURE_THRESHOLD,
        DEFAULT_TEMPERATURE_THRESHOLD,
    )
...
async def test_parameter_override_affects_evaluation(coord_factory) -> None:
    """Setting a temperature threshold override above current temp suppresses cooling."""
    ...
    # temperature=25, generation=6000 (above cool threshold) — default threshold is 24
    ...
    # Raise threshold above current temp — auto mode turns system off since temp is below threshold
    # async_set_parameter internally calls async_run_evaluation, so coordinator.data is already updated
    await coordinator.async_set_parameter(CONF_TEMPERATURE_THRESHOLD, 30.0)
