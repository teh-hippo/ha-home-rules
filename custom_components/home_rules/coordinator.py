...
class HomeRulesCoordinator(DataUpdateCoordinator[CoordinatorData]):
    ...
    def get_parameter(self, key: str, default: float) -> float:
        """Return the current value for a threshold parameter (runtime override or config entry option)."""
    ...
    async def async_set_parameter(self, key: str, value: float) -> None:
        """Set a runtime threshold override and trigger re-evaluation."""
    ...
    def parameters(self) -> RuleParameters:
        return RuleParameters(
            ...
            grid_usage_delay=int(self.config_entry.options.get(c.CONF_GRID_USAGE_DELAY, c.DEFAULT_GRID_USAGE_DELAY)),
            reactivate_delay=int(self.config_entry.options.get(c.CONF_REACTIVATE_DELAY, c.DEFAULT_REACTIVATE_DELAY)),
